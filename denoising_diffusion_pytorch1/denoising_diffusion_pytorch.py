...
class GaussianDiffusion(nn.Module):
    ...
    def p_sample(self, x, t: int, x_self_cond=None, clip_denoised=True):
        ...
        betas_t = extract(self.betas, batched_times, x.shape)
        sqrt_one_minus_alphas_cumprod_t = extract(
            self.sqrt_one_minus_alphas_cumprod, batched_times, x.shape
        )
        sqrt_recip_alphas_t = extract(self.sqrt_recip_alphas, batched_times, x.shape)

        model_mean = sqrt_recip_alphas_t * (
                x - betas_t * self.model(x, batched_times) / sqrt_one_minus_alphas_cumprod_t
        )
        posterior_variance_t = extract(self.posterior_variance, batched_times, x.shape)
...
if __name__ == '__main__':
    unet = Unet(dim=64, channels=3, dim_mults=(1, 2, 4, 8))
    x = torch.randn([16, 3, 28, 28])
    t = torch.randint(0, 1000, (16,)).long()
    unet(x,t)
